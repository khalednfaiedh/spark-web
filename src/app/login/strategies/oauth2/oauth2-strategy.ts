...
export class NbOAuth2AuthStrategy extends NbAuthStrategy {
  ...
  authenticate(data?: any): Observable<NbAuthResult> {
    ...
    if (this.getOption('token.grantType') === NbOAuth2GrantType.PASSWORD) {
      return this.passwordToken(data.login, data.password)
    } else {
  ...
  passwordToken(login: string, password: string): Observable<NbAuthResult> {
    const module = 'token';
    const url = this.getActionEndpoint(module);
    const requireValidToken = this.getOption(`${module}.requireValidToken`);
    ...
    return this.http.post(url, new AuthenticationRequest(login, password), { headers: headers })
      .pipe(
        map((res) => {
          return new NbAuthResult(
            true,
