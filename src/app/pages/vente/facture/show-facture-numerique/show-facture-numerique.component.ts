...
import { NbWindowRef, NbDateService, NbToastrService, NbGlobalPhysicalPosition } from '@nebular/theme';
...
@Component({
  selector: 'ngx-show-facture-numerique',
  templateUrl: './show-facture-numerique.component.html',
  styleUrls: ['./show-facture-numerique.component.scss']
})
export class ShowFactureNumeriqueComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.serviceFacture.getFactureById(+this.id).subscribe(dataF => {
      this.facture = dataF
    })
  ...
  calculeTTC() {
    this.serviceTaxe.getTaxebyId(this.facture.idTaxe).subscribe(data => {
      ...
      if (this.taxe.type == "Montant fixe") {
        this.commande.sommeTotaleavecTVA = this.commande.sommeTotaleavecTVA + this.taxe.pourcentage
      } else {
        this.commande.sommeTotaleavecTVA = this.commande.sommeTotaleavecTVA + (this.commande.sommeTotaleavecTVA * this.taxe.pourcentage) / 100
      }
  ...
  calculeMontantPayer() {
    this.commande.sommeFinale = this.commande.sommeTotaleavecTVA + this.commande.fraislivraison + this.commande.autrefrais
  }
  calculeMontantReste() {
    this.commande.montantReste = this.commande.sommeFinale - this.commande.montantpayer
    console.log("this.commande.montantReste", this.commande.montantReste)
  }
