import { Component, OnInit } from '@angular/core';
import { DeductionLogementModel } from '../DeductionLogement.model';
import { DeductionLogementService } from '../DeductionLogement.service';
import { Router } from '@angular/router';
import { NbWindowRef } from '@nebular/theme';
import { MoisService } from '../../../paie/mois/mois.service';
import { ContratService } from '../../contrat.service';
import { ContratModel } from '../../contrat.model';

@Component({
  selector: 'ngx-form-deduction',
  templateUrl: './form-deduction.component.html',
  styleUrls: ['./form-deduction.component.scss']
})
export class FormDeductionComponent implements OnInit {

  
  buttonValue: string;
  deduction: DeductionLogementModel;
  annee = [ "2015", "2016", "2017", "2018", " 2019", "2020", "2021", "2022", "2023", "2024", " 2025", "2026", "2027", "2028", " 2030", "2031 ", "2032", "2033", "2034", "2035", "2036", "2037", "2038", "2039", "2040", " 2041","2042", "2043", "2044", "2045", "2046", "2047", "2048", "2049", "2050"]
 //mois = ["01","02","03","04","05","06","07","08","09","10","11","12"]
D = ['Déduction logement']
mois = []
contrat : Promise<ContratModel>
idEntreprise : number
mensualitieNumber : any
  constructor( private moisService : MoisService,private contratService : ContratService,
    private service: DeductionLogementService, private router: Router, private windowRef: NbWindowRef) {
  }

  ngOnInit() {
   let idContrat = localStorage.getItem('idContrat')
   this.asyncNgOnInit(idContrat)

    this.deduction = new DeductionLogementModel();
    let e = localStorage.getItem('e');
    if (e === '0') {
      this.buttonValue = 'Ajouter';
    }
    if (e === '1') {
      let idDeduction = localStorage.getItem('idDeduction');
    
      this.buttonValue = 'Modifier';
      this.service.getDeductionLogementById(+idDeduction).subscribe(
        data => { this.deduction = data;
         },
        error => {  });
    }
  }
  getContrat(idC) {
    return this.contratService.getContratsById(idC).toPromise();
  }
  getMensualiteNumber(idE){
    this.moisService.mensualiteNumber(idE).subscribe(
      data => { this.mensualitieNumber = data ;
        switch (this.mensualitieNumber) {
          case 13:  this.mois = ["01","02","03","04","05","06","07","08","09","10","11","12","13"]; break;
          case 14:  this.mois = ["01","02","03","04","05","06","07","08","09","10","11","12","13","14"]; break;
          case 15:  this.mois = ["01","02","03","04","05","06","07","08","09","10","11","12","13","14","15"]; break;
          case 16:  this.mois = ["01","02","03","04","05","06","07","08","09","10","11","12","13","14","15","16"]; break;
          case 12:  this.mois = ["01","02","03","04","05","06","07","08","09","10","11","12"]; break;
        } 
      });
  }
  
  async asyncNgOnInit(idContrat) {
   this.contrat =  this.getContrat(idContrat) ;
   this.idEntreprise =(await this.contrat).idEntreprise ;
   this.getMensualiteNumber(this.idEntreprise);
  }


  onAddA() {
    let e = localStorage.getItem('e');
    let idC = localStorage.getItem('idContrat');
    if (e === '0') {
      this.service.addDeductionLogement(+idC, this.deduction).subscribe(
        data => {
          localStorage.removeItem('e');
          this.router.navigateByUrl('/', { skipLocationChange: true }).then(() =>
            this.router.navigate(["/pages/rh/deduction"]));
          this.windowRef.close();
        },
        error => {
         
        });
    }
    if (e === '1') {
      this.service.updateDeductionLogement(+idC, this.deduction).subscribe(
        data => {
          localStorage.removeItem('e');
         
          this.router.navigateByUrl('/', { skipLocationChange: true }).then(() =>
            this.router.navigate(["/pages/rh/deduction"]));
          this.windowRef.close();
        },
        error => {
       
        });
    }
  }
}